...
const options: swaggerJsdoc.Options = {
  definition: {
    ...
    paths: {
      ...
      '/api/v1/residents/{residentId}/charting/{type}': {
        get: { tags: ['Charting'], summary: 'List charting records by type', security: [{ BearerAuth: [] }], parameters: [{ name: 'residentId', in: 'path', required: true, schema: { type: 'integer' } }, { name: 'type', in: 'path', required: true, schema: { type: 'string', enum: ['vitals','allergies','medications','care-plans','events','progress-notes','services','tickets','inventory','incidents','pain-scale','face-to-face'] } }], responses: { '200': { description: 'List of records' } } },
        post: { tags: ['Charting'], summary: 'Create a charting record', security: [{ BearerAuth: [] }], parameters: [{ name: 'residentId', in: 'path', required: true, schema: { type: 'integer' } }, { name: 'type', in: 'path', required: true, schema: { type: 'string' } }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object' } } } }, responses: { '201': { description: 'Created' } } },
      },
      '/api/v1/residents/{residentId}/charting/{type}/{recordId}': {
        put: { tags: ['Charting'], summary: 'Update a charting record', security: [{ BearerAuth: [] }], parameters: [{ name: 'residentId', in: 'path', required: true, schema: { type: 'integer' } }, { name: 'type', in: 'path', required: true, schema: { type: 'string' } }, { name: 'recordId', in: 'path', required: true, schema: { type: 'integer' } }], responses: { '200': { description: 'Updated' } } },
        delete: { tags: ['Charting'], summary: 'Delete a charting record', security: [{ BearerAuth: [] }], parameters: [{ name: 'residentId', in: 'path', required: true, schema: { type: 'integer' } }, { name: 'type', in: 'path', required: true, schema: { type: 'string' } }, { name: 'recordId', in: 'path', required: true, schema: { type: 'integer' } }], responses: { '200': { description: 'Deleted' } } },
      },
      '/api/v1/attendance/daily': { get: { tags: ['Attendance'], summary: 'Get daily attendance roster', security: [{ BearerAuth: [] }], parameters: [{ name: 'date', in: 'query', required: true, schema: { type: 'string', format: 'date' } }], responses: { '200': { description: 'Daily roster' } } } },
      '/api/v1/attendance/weekly': { get: { tags: ['Attendance'], summary: 'Get weekly attendance roster', security: [{ BearerAuth: [] }], parameters: [{ name: 'weekStart', in: 'query', required: true, schema: { type: 'string', format: 'date' } }], responses: { '200': { description: 'Weekly roster' } } } },
      '/api/v1/attendance/check-in': { post: { tags: ['Attendance'], summary: 'Check in a resident', security: [{ BearerAuth: [] }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', properties: { residentId: { type: 'integer' }, date: { type: 'string' } }, required: ['residentId'] } } } }, responses: { '201': { description: 'Checked in' } } } },
      '/api/v1/attendance/check-out': { post: { tags: ['Attendance'], summary: 'Check out a resident', security: [{ BearerAuth: [] }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', properties: { residentId: { type: 'integer' }, date: { type: 'string' } }, required: ['residentId'] } } } }, responses: { '200': { description: 'Checked out' } } } },
      '/api/v1/attendance/mark-absent': { post: { tags: ['Attendance'], summary: 'Mark a resident absent', security: [{ BearerAuth: [] }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', properties: { residentId: { type: 'integer' }, reason: { type: 'string' }, date: { type: 'string' }, notes: { type: 'string' } }, required: ['residentId', 'reason'] } } } }, responses: { '201': { description: 'Marked absent' } } } },
      '/api/v1/residents/{residentId}/discharge-full': {
        post: { tags: ['Discharge'], summary: 'Create full discharge record (4-step wizard)', security: [{ BearerAuth: [] }], parameters: [{ name: 'residentId', in: 'path', required: true, schema: { type: 'integer' } }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object' } } } }, responses: { '201': { description: 'Discharge record created' } } },
        get: { tags: ['Discharge'], summary: 'Get discharge record', security: [{ BearerAuth: [] }], parameters: [{ name: 'residentId', in: 'path', required: true, schema: { type: 'integer' } }], responses: { '200': { description: 'Discharge record' } } },
      },
      '/api/v1/residents/{residentId}/setup': { get: { tags: ['Patient Setup'], summary: 'Get patient setup wizard state', security: [{ BearerAuth: [] }], parameters: [{ name: 'residentId', in: 'path', required: true, schema: { type: 'integer' } }], responses: { '200': { description: 'Setup state' } } } },
      '/api/v1/residents/{residentId}/setup/step/{stepKey}': { put: { tags: ['Patient Setup'], summary: 'Save wizard step data', security: [{ BearerAuth: [] }], parameters: [{ name: 'residentId', in: 'path', required: true, schema: { type: 'integer' } }, { name: 'stepKey', in: 'path', required: true, schema: { type: 'string' } }], responses: { '200': { description: 'Step saved' } } } },
      '/api/v1/residents/{residentId}/setup/complete/{stepIndex}': { post: { tags: ['Patient Setup'], summary: 'Mark wizard step complete', security: [{ BearerAuth: [] }], parameters: [{ name: 'residentId', in: 'path', required: true, schema: { type: 'integer' } }, { name: 'stepIndex', in: 'path', required: true, schema: { type: 'integer' } }], responses: { '200': { description: 'Step completed' } } } },
    },
  },
  apis: [],
};
